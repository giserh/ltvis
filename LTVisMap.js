...
function addJSONAreaSummaryLayer(geoJSON) {
  ...
  function click(e) {
    ...
    // the timeline chart module. Will need to store the clicked
    LTVis.displayFeatureSummaryData(e.target.feature);
  }
}

function addCanvasLayer(layer) {
  if(canvasLayer !== null) {
...
return {
  ...
  addJSONAreaSummaryLayer: function(geoJSON) {
    addJSONAreaSummaryLayer(geoJSON);
  },
  ...
  submitDrawnPolygons: function() {
    ...
    addJSONAreaSummaryLayer(drawnItems.toGeoJSON());
  },
  ...
  init: function(callback) {
    ...
    $.get('configFiles/mapConfig.yaml', null, function(data) {
      ...
      // FIXME Right now this requires that there be a 
      // div in the HTML with the id "map". Might be better to 
      // pass in that id from main.js
